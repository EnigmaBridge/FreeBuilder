...
import com.enigmabridge.ebuilder.processor.util.Shading;
...
public class Util {
  ...
  static boolean erasesToAnyOf(DeclaredType type, QualifiedName... possibilities) {
    ...
    for (QualifiedName possibility : possibilities) {
      if (Shading.unshadedName(possibility.toString()).equals(erasedType)) {
        return true;
      }
    }
    return false;
  ...
  static boolean erasesToAnyOf(DeclaredType type, Class<?>... possibilities) {
    ...
    for (Class<?> possibility : possibilities) {
      if (Shading.unshadedName(possibility.getName()).equals(erasedType)) {
        return true;
      }
    }
    return false;
