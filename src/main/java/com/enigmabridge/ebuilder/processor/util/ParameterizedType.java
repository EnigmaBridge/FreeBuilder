...
public class ParameterizedType extends Excerpt {
  ...
  public Excerpt constructor() {
    return Excerpts.add(
        "new %s%s",
        qualifiedName,
        isParameterized() ? SourceLevel.diamondOperator(Excerpts.join(", ", typeParameters)) : "");
  }
  ...
  private static boolean extendsObject(TypeParameterElement element) {
    ...
    TypeElement bound = ModelUtils.maybeAsTypeElement(getOnlyElement(element.getBounds())).orNull();
    if (bound == null) {
      return false;
    }
