...
import com.enigmabridge.ebuilder.processor.util.testing.CompilationException;
...
public class SetPropertyFactoryTest {
  ...
  public void testImmutableSetProperty() {
    assumeTrue("Guava available", features.get(GuavaLibrary.GUAVA).isAvailable());
    behaviorTester
        .with(new Processor(features))
        .with(new SourceBuilder()
            .addLine("package com.example;")
  ...
  public void testFromReusesImmutableSetInstance() {
    assumeTrue(features.get(GuavaLibrary.GUAVA).isAvailable());
    behaviorTester
        .with(new Processor(features))
        .with(SET_PROPERTY_AUTO_BUILT_TYPE)
        .with(new TestBuilder()
